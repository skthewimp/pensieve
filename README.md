...
- Session-based chat history with fresh chats by default.
- Copy/share export for assistant answers as Markdown with cited note context.
...
- **Memory**: combined notes and knowledge browser. Sections include Notes,
  Topics, and Map so Wiki and Mindmap are discoverable without crowding the tab
  bar.
- **Chat**: ask questions against the saved note corpus. New chats start with
  an empty composer, while previous sessions stay available in history. Assistant
  answers can be copied or shared as Markdown bundles for use in another LLM.
- **Review**: combined review workspace. Sections include Queue, Insights, and
  Tensions for pending insights, topic pages, open loops, contradictions, and
  corpus summaries.
