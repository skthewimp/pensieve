...
- Notes tab.
- Wiki tab.
- Insights tab with local corpus summaries.
- Mindmap tab grouped by themes.
...
- Review/dismiss UI for contradictions.
...
- **Wiki**: browsable note wiki. Repeated themes are shown first, ordered by
  note count. One-off themes are hidden behind a `Show one-off themes` row.
  Search covers note title, summary, body, and themes.
- **Insights**: local dashboard for recurring themes, recently active themes,
  open-loop-like notes, decision/plan notes, and unresolved contradictions.
...
- **Mindmap**: theme-grouped note browser.
...
iCloud Drive or another durable location. Restore is not implemented yet.
