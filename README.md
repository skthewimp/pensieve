...
- Manual topic cleanup into canonical themes and generated Wiki topics.
...
- OpenAI provider.
- Topic cleanup diagnostics, previews, and rerun controls.
...
- **Topic cleanup** runs only when `Settings -> Clean Up Topics` is tapped.
  It consolidates overlapping note themes and generates source-backed Wiki
  topic pages. The current implementation asks the LLM for a compact taxonomy,
